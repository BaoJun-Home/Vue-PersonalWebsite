      ...
      [`rows|${limit || 10}`]: [
        {
          ...
          "scanNumber|1000-3000": 0,
          "commentNumber|100-300": 0,
          "thumb|1": ["@image(200x150, @color, @color, png, @natural)", null],
          createDate: "@date(T)",
        },
      ],
    },
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
  ...
  data: {
    ...
    "thumb|1": ["@image(200x150, #000, #fff, png, random-image)", null],
  },
});
...
Mock.mock(/^\/api\/comment\/?(\?.+)?$/, "get", (options) => {
  const { limit, blogId } = qs.parse(options.url);
  return Mock.mock({
    code: 0,
    msg: "",
    data: {
